...
use actix_web::{error::{ErrorInternalServerError}, Error, FromRequest};
use db_core::{models::user::UserCredential, Role};
...
impl FromRequest for Context{
    ...
    fn from_request(req: &actix_web::HttpRequest, _: &mut actix_web::dev::Payload) -> Self::Future {
        ...
        let Ok(ser) = session else {
            return ready(Err(ErrorInternalServerError("Fail to Deserialize Session!")))
        };
