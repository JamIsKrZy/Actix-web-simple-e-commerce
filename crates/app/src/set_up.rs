...
pub mod shuttle {
    ...
    pub fn from_base64_secrets(
        ...
        let secret_key_str = secrets.get("SECRET_KEY")
...
pub mod seed {
    ...
    use db_core::{models::{self, user::{AddUser, RawPassword}}, DbPoolExtract};
    ...
    impl<'a> DbPoolExtract<sqlx::Postgres> for DbWrap<'a>{
        ...
        fn transaction(&self) -> std::pin::Pin<Box<impl Future< Output = Result<sqlx::Transaction<'static, sqlx::Postgres>, sqlx::Error>>>> {
    ...
    pub async fn seed_super_admin(
        ...
        let admin_username = secrets.get("SUPER_ADMIN")
        ...
        let super_admin = sqlx::query_scalar!(
            ...
            .await
            .unwrap_or_else(|e| 
                panic!("Failed to query: {}", e.to_string())
            );
        

        if super_admin.is_none() {

            let admin_password = secrets.get("SUPER_ADMIN_PASSWORD")
            ...
            let admin = admin_info.hash_password(hasher)
                .await
