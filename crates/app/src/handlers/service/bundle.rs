...
pub mod admin {
    ...
    pub fn scope(cfg: &mut ServiceConfig) {
        bind_scope_handlers!(cfg, "/bundles", create_bundle, list_bundles);
    }
    ...
    async fn list_bundles(
        ...
        let list = bundles::Bmc::get_full_list(page, db).await?;

        match acpt {
            extension::extractor::Accepted::Json => Ok(HttpResponse::Ok().json(json!({
                "list": list
            }))),
            extension::extractor::Accepted::Html => Ok(HttpResponse::Ok()
                .body(template_format::manage_page::BundleList::from(list).to_string())),
        }
    }
}
